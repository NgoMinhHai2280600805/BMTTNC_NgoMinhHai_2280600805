...
def md5(message):
    ...
    # Bảng các giá trị hằng số k
    k = [int(abs(math.sin(i + 1)) * (2**32)) & 0xFFFFFFFF for i in range(64)]

    # Bảng số lượng dịch trái cho mỗi vòng
    r = [
    ...
    # Thêm bit '1' vào cuối message (theo chuẩn)
    message += b'\x80'

    # Thêm các bit 0 sao cho độ dài mod 64 bằng 56
    while (len(message) % 64) != 56:
        message += b'\x00'

    # Thêm độ dài ban đầu của message dưới dạng 64-bit little-endian
    ...
    # Kết quả cuối cùng (little-endian)
